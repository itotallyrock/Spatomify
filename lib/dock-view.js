...
export default class DockView {
  ...
  async updateArtwork () {
    try {
      await this.song.album.fetchArtwork()
      this.getAlbumArtElement().style.backgroundImage = this.song.album.artwork.generateCSS()
    } catch (albumArtFetchError) {
      NotificationManager.addWarning('Failed to fetch artwork', { description: albumArtFetchError.message })
