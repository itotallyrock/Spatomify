...
export default class SongElement {
  ...
  async updateImage () {
    await this.song.artwork.fetch()
    this.getAlbumArtElement().style.backgroundImage = this.song.artwork.generateCSS()
  }

  updateTitle () {
    this.getTitleElement().innerText = this.song.track.name
  }

  updateArtist () {
    this.getArtistElement().innerText = this.song.artist.name
  }
